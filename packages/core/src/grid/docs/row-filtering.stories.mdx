...
To use row filtering leverage the `cds-search` or `cds-datalist` components. With `cds-action-button`
...
<cds-grid>
  <cds-grid-column>
    Stock
    <cds-action-button id="id-filter-demo" shape="filter" aria-label="search available stocks"></cds-action-button>
