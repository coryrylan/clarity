...
To use row filtering leverage the `cds-search` or `cds-datalist` components. With `cds-control-action`
...
<cds-grid>
  <cds-grid-column>
    Stock
    <cds-control-action id="id-filter-demo" shape="filter" aria-label="search available stocks"></cds-control-action>
