...
<Meta title="Data Grid (Preview)/Swappable Rows" />
...
Keyboard navigation support is available by applying the `cds-draggable="handle"` attribute to a `cds-control-action`.
...
<cds-grid>
  ...
  <cds-grid-row id="1" draggable="true">
    <cds-grid-cell>
      <cds-control-action cds-draggable="handle" shape="drag-handle" aria-label="sort row"></cds-control-action>
    </cds-grid-cell>
    <cds-grid-cell>1</cds-grid-cell>
