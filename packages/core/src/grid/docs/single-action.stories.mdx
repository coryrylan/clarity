...
Single Actions can be applied to `cds-grid-row` by leveraging the `cds-control-action`.
The `cds-control-action` can open a context dropdown per row to allow the user to
