...
Single Actions can be applied to `cds-grid-row` by leveraging the `cds-action-button`.
The `cds-action-button` can open a context dropdown per row to allow the user to
