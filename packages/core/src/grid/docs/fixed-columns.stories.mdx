...
of the position by using a `cds-control-action` to set the position state of a given column.
...
<cds-grid>
  <cds-grid-column position="fixed">
    Stock
    <cds-control-action aria-label="pin stock column">
  ...
  <cds-grid-column position="initial">
    About
    <cds-control-action aria-label="pin about column">
