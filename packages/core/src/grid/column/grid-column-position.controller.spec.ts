...
describe('grid-column-position.controller', () => {
  ...
  it('should support fixed left position', async () => {
    ...
    expect(getComputedStyle(firstCell).getPropertyValue('--border-right').trim()).toBe('calc((1 / 20) * 1rem) solid hsl(198, 14%, 82%)');
  });
  ...
  it('should support fixed right position', async () => {
    ...
    expect(getComputedStyle(lastCell).getPropertyValue('--border-left').trim()).toBe('calc((1 / 20) * 1rem) solid hsl(198, 14%, 82%)');
  });
