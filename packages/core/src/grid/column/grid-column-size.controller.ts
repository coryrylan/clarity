...
export class GridColumnSizeController {
  ...
  private updateResizedColumnWidth(width: number) {
    const updatedWidth = Math.max(isNumericString(this.host.width) ? parseInt(this.host.width) : 44, parseInt(getComputedStyle(this.host).width) + width);
